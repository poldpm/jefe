...
// ------------- l'escola viatja amb les agendes, no a sobre de les agendes
/* Les agendes de l'escola van per un pont —una volta sencera a un altre Apps
   Script, tres segons— i s'hi anava a part: se sumaven al que costava la resta.
   Ara van a la MATEIXA tirada que les teves, i el que es paga és la petició més
   lenta i no la suma.
   El que es prova: que hi vagi de debò a dins, que les cites de l'escola
   surtin, i que si el pont contesta malament es torni a provar pel camí de
   sempre —que és el que hi havia i funcionava. */
console.log('\nL\'escola, a la mateixa tirada que les teves agendes');
{
  const ctx = carregaTotElServidor();
  ctx.Config = { zonaHoraria: () => 'Europe/Madrid', get: () => null, getNum: (k, d) => d };
  ctx.Log = { info() {}, avis() {}, error() {} };
  ...
  ctx.Dades = {
    llegeix: () => [
      { id: 'meu@g.com', nom: 'El meu', color: '#000', mostra: 'SI', principal: 'SI',
        meu: 'SI', pont: 'NO', ordre: 1 },
      { id: 'escola@w.com', nom: 'Escola', color: '#a00', mostra: 'SI', principal: 'NO',
        meu: 'NO', pont: 'SI', ordre: 2 }
    ],
    un: () => null, perId: () => null, insereix: (f, x) => x, actualitza: () => null
  };

  const CITA_ESCOLA = {
    id: 'esc1', calendari: 'escola@w.com', calendariNom: 'Escola', color: '#a00',
    titol: 'Claustre', lloc: '', nota: '', data: '2026-08-04', dataFi: '2026-08-04',
    totElDia: false, hora: '09:00', horaFi: '10:00', passat: false, minuts: 60
  };

  let peticionsFetes = [], solesDelPont = 0, pontRespon = 200;
  ctx.CalendariPont = {
    hiEs: () => true,
    peticioEsdeveniments: () => ({ url: 'https://escola/exec', method: 'post',
                                   payload: '{}', muteHttpExceptions: true }),
    llegeix: (r) => {
      if (r.getResponseCode() !== 200) throw new Error('l\'escola ha dit que no');
      return [CITA_ESCOLA];
    },
    esdeveniments: () => { solesDelPont++; return [CITA_ESCOLA]; }
  };
  ctx.UrlFetchApp = { fetchAll: (p) => {
    peticionsFetes = p.map((x) => x.url);
    return p.map((x) => ({
      getResponseCode: () => (/escola/.test(x.url) ? pontRespon : 200),
      getContentText: () => JSON.stringify({ items: [] })
    }));
  } };

  const l = ctx.Calendari.rang('2026-08-01', '2026-08-31');
  cal('el pont va dins la mateixa tirada que les agendes',
      peticionsFetes.length === 2 && /escola/.test(peticionsFetes[1]),
      JSON.stringify(peticionsFetes));
  cal('i les cites de l\'escola hi surten',
      l.length === 1 && l[0].id === 'esc1', JSON.stringify(l));
  cal('sense haver-hi anat a part', solesDelPont === 0, String(solesDelPont));

  /* I si el pont contesta malament, es torna a provar pel camí de sempre. */
  pontRespon = 401;
  solesDelPont = 0;
  const l2 = ctx.Calendari.rang('2026-08-01', '2026-08-31');
  cal('si de cop no ha anat, s\'hi torna pel camí de sempre',
      solesDelPont === 1 && l2.length === 1,
      JSON.stringify({ soles: solesDelPont, quantes: l2.length }));
}
